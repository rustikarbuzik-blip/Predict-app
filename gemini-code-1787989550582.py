        ...
        if not input_vsegpt_key:
            ...
            for i, match in enumerate(matches, 1):
                ...
                with st.spinner(f"Анализ {i}/{len(matches)} ({home} — {away})..."):
                    ...
                    prompt = f"""
                    Ты профессиональный спортивный аналитик. Твоя задача — дать РАЗНЫЕ и уникальные типы ставок для матча: {home} vs {away}.
                    Время: {time_str} (Уфа).

                    🚨 ЖЕСТКИЕ ПРАВИЛА (НАРУШЕНИЕ — ОШИБКА):
                    1. КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО ставить индивидуальный тотал больше 1.5 (ИТБ 1.5) подряд на разные матчи! Если в предыдущем матче был ИТБ, здесь ОБЯЗАНЫ быть другие рынки (ИТМ, фора, тотал меньше, обе забьют).
                    2. Чередуй маркеты: используй форы (0, +1, -1.5), тоталы меньше (ТМ 2.5, ТМ 3.5), индивидуальные тоталы меньше (ИТМ 1.0, ИТМ 1.5) или комбинированные исходы.
                    3. Запрещены аббревиатуры П1/П2/ИТ1. Всегда пиши полное название команды.
                    ...
                    ИТ: [{home} или {away} ИТМ(1.0) / ИТМ(1.5) / ИТБ(1.0) — ВЫБЕРИ РАЗНОЕ!]
                    БОЛЕЕ_АГРЕССИВНО: [{home} или {away} с форой / сухая победа / ОЗ]
                    ОСТОРОЖНАЯ_СТАВКА: [{home} 1X / {away} X2 / плюсовая фора / тотал меньше]
